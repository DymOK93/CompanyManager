...
private:
	...
	struct TaskManagement {
		...
		TaskManager<CompanyManager> modify_xml;										//Стандартная емкость очереди команд
...
private:
	...
	void closeEvent(QCloseEvent* event) override;					//Перехват события для запроса соранения файла
private:
	...
	void internal_fatal_error_msg(
		QString title,
		const std::string& error_definition = ""					//Поскольку operator+= выполняет копирование, а лог хранит std::string, передача
	) const noexcept;												//по ссылке с дальнейшим копированием из std::string_view::data() выгоднее двух созданий QString
