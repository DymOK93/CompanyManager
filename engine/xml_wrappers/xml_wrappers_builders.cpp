...
EmployeeBuilder& EmployeeBuilder::SetMiddleName(
    std::string middle_name) noexcept {
...
Employee EmployeeBuilder::Assemble() {
  ...
  return xml::DocumentNodeBuilder()
      ...
      .SetChildren(element_builder.SetName("surname")
                       .SetText(move(m_personal_data.surname))
                       .Assemble(),
                   element_builder.SetName("name")
                       .SetText(move(m_personal_data.name))
                       .Assemble(),
                   element_builder.SetName("middleName")
                       .SetText(move(m_personal_data.middle_name))
                       .Assemble(),
                   element_builder.SetName("function")
                       .SetText(move(m_personal_data.function))
                       .Assemble(),
                   element_builder.SetName("salary")
                       .SetText(to_string(m_personal_data.salary))
                       .Assemble())
      .Assemble();
}
...
DepartmentBuilder& DepartmentBuilder::InsertEmployees(
...
Department DepartmentBuilder::Assemble() {
  ...
  Department department{xml::DocumentNodeBuilder()
                            .SetName("department")
                            .SetAttribute("name", move(m_name))
                            .SetAllocator(alloc)
                            .SetChildren(xml::DocumentNodeBuilder()
                                             .SetName("employments")
                                             .SetAllocator(alloc)
                                             .SetChildren()
                                             .Assemble())
...
Company CompanyBuilder::Assemble() {
  Company company{xml::DocumentNodeBuilder()
                      .SetName("departments")
                      .SetAllocator(take_allocator())
                      .SetChildren()
                      .Assemble()};
