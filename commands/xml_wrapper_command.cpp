...
namespace command {
	namespace xml_wrapper {
		...
		RenameDepartment::RenameDepartment(
			...
			string new_name
		) noexcept : MyBase(cm), m_current_name(current_name), m_new_name(move(new_name))
		{
		}
		...
		any RenameDepartment::Execute() {
			...
			auto result{										
				get_tree_ref().RenameDepartment(
					m_current_name, move(m_new_name)
				)
			};
		...
		any RemoveDepartment::Cancel() {
			...
			department_it it;
			if (m_before) {
				it = tree.InsertDepartment(
					*m_before,
					move(get_wrapper())
				);
			}
			else {
				it = tree.AddDepartment(move(get_wrapper()));					//—охран€ем наименование подразделени€
			}
			m_value = it->GetName();
			return it;
		}
