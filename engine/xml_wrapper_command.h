...
class ModifyCompany : public ModifyCommand<ConcreteCommand> {
 ...
 public:
  ModifyCompany(CompanyManager& cm,
                const SubstituteTy&
                    substitute)  // SubstituteTy должен дЄшево копироватьс€
      noexcept(std::is_nothrow_copy_constructible_v<SubstituteTy>)
      : MyBase(cm), m_value(substitute) {}
  ModifyCompany(CompanyManager& cm,
                WrapperTy&& value) noexcept  //Ёкономи€ вызова move c-tor
      : MyBase(cm), m_value(std::move(value)) {}

 protected:
  WrapperTy& get_wrapper() { return std::get<WrapperTy>(m_value); }
  SubstituteTy& get_substitute() { return std::get<SubstituteTy>(m_value); }

 protected:
  std::variant<WrapperTy, SubstituteTy> m_value;
};

class AddDepartment : public ModifyCompany<AddDepartment,
                                           wrapper::Department,
                                           wrapper::string_ref> {
 public:
  using MyBase =
      ModifyCompany<AddDepartment,
                    wrapper::Department,
                    wrapper::string_ref>;  //’ранить итератор нельз€, т.к. он
                                           //может инвалидироватьс€
  using command_holder =
...
class InsertDepartment : public ModifyCompany<InsertDepartment,
 ...
 public:
  using MyBase =
      ModifyCompany<InsertDepartment, wrapper::Department, wrapper::string_ref>;
  using command_holder = MyBase::command_holder;
...
class RemoveDepartment : public ModifyCompany<RemoveDepartment,
 ...
 public:
  using MyBase =
      ModifyCompany<RemoveDepartment, wrapper::Department, wrapper::string_ref>;
  using command_holder = MyBase::command_holder;
...
/************************************************************************************************************************
ѕочему можно в EmployeePersonalFile хранить FullNameRef, не опаса€сь
инвалидации? ѕол€ FullNameRef ссылаютс€ на дочерние узлы XML-узла типа Employee.
 онтейнер дочерних узлов - xml::container_t = перемещаетс€ без перемещени€
элементов, кроме того, он содержит не сами узлы, а node_holder'ы. ѕоскольку все
операции вставки и удалени€ извлекают данные из хранилищ, а не удал€ют
непосредственно, инвалидируютс€ лишь итераторы, а все текстовые дочерние узлы
XML-Employee (surname, name, middle_name, function, salary) продолжат быть
доступны.
*************************************************************************************************************************/
...
class ModifyEmployeeFields : public ModifyCommand<ConcreteCommand> {
 ...
 public:
  ModifyEmployeeFields(
      ...
      SwappedFieldTy
          value) noexcept(std::is_nothrow_move_constructible_v<SwappedFieldTy>)
 ...
 protected:
  SwappedFieldTy& get_value() noexcept { return m_value; }
...
template <class ConcreteCommand>
class ModifyEmployeeTextFields
    : public ModifyEmployeeFields<ConcreteCommand, std::string> {
 public:
...
class ChangeEmployeeSurname
    : public ModifyEmployeeTextFields<ChangeEmployeeSurname> {
 public:
...
class ChangeEmployeeMiddleName
    : public ModifyEmployeeTextFields<ChangeEmployeeMiddleName> {
 public:
...
class ChangeEmployeeFunction
    : public ModifyEmployeeTextFields<ChangeEmployeeFunction> {
 public:
...
class UpdateEmployeeSalary
    : public ModifyEmployeeFields<UpdateEmployeeSalary,
                                  wrapper::Employee::salary_t> {
 public:
  using MyBase =
      ModifyEmployeeFields<UpdateEmployeeSalary, wrapper::Employee::salary_t>;
  using command_holder = MyBase::command_holder;
...
template <class ConcreteCommand>
class ModifyDepartment : public ModifyCompany<ConcreteCommand,
                                              wrapper::Employee,
                                              wrapper::FullNameRef> {
 public:
  using MyBase =
      ModifyCompany<ConcreteCommand, wrapper::Employee, wrapper::FullNameRef>;
  using command_holder = typename MyBase::command_holder;

 public:
  ModifyDepartment(CompanyManager& cm,
                   wrapper::string_ref department,
                   wrapper::Employee&& employee) noexcept
      : MyBase(cm, std::move(employee)), m_department(department) {}
  ModifyDepartment(CompanyManager& cm,
                   wrapper::string_ref department,
                   const wrapper::FullNameRef& full_name) noexcept
      : MyBase(cm, full_name), m_department(department) {}
