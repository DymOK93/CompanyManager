...
EmptyPathChecker::EmptyPathChecker(string_view path) : m_path(path) {}
...
EmptyPathChecker::chain_worker_holder EmptyPathChecker::make_instance(
    string_view path) {
...
OpenerForReading::chain_worker_holder OpenerForReading::make_instance(
    ifstream& in,
    string_view path) {
  return allocate_instance(in, path);
}

XmlReader::XmlReader(xml::Reader& reader,
                     xml::Document& target,
...
void XmlReader::Process(Result& result) {
  ...
  result = m_reader.Fail() ? Result::FileIOError : Result::Success;
...
OpenerForWriting::chain_worker_holder OpenerForWriting::make_instance(
    ofstream& out,
    string_view path) {
...
void XmlWriter::Process(Result& result) {
  ...
  result = m_writer.Fail() ? Result::FileIOError : Result::Success;
  if (result == Result::Success) {
    MyBase::pass_on(result);
  }
}

XmlWriter::chain_worker_holder XmlWriter::make_instance(
    xml::Writer& writer,
...
PipelineBuilder& PipelineBuilder::OpenForReading(ifstream& in,
                                                 string_view path) {
  return MyBase::attach_node(OpenerForReading::make_instance(in, path));
}

PipelineBuilder& PipelineBuilder::OpenForWriting(ofstream& out,
                                                 string_view path) {
...
PipelineBuilder& PipelineBuilder::WriteXml(xml::Writer& writer,
                                           const xml::Document& source) {
