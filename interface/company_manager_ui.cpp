...
CompanyManagerUI::CompanyManagerUI(QWidget* parent) : QMainWindow(parent) {
  m_gui->setupUi(this);
...
std::tuple<Types&...>, чтобы гарантированно воспользоваться преимуществами SSO.
********************************************************************************************************************************/
...
bool CompanyManagerUI::undo() {  //Порядок отмены операций един для команд
  ...
  PipelineBuilder()
      .AddHandler([this](bool& wrapper_success) {
...
bool CompanyManagerUI::redo() {  //Порядок повторного выполнения операций
  ...
  bool wrapper_success{false};
  bool reversed_order{
      *m_tasks->modify_xml.GetLastProcessedPurpose() ==
...
bool CompanyManagerUI::new_department() {
  ...
  bool append{
      ...
      || *m_tree_model->GetItemType(current_selection_idx) !=
             CompanyTreeModel::ItemType::Department  //Валидность индекса уже
                                                     //проверена
      || current_selection_idx.row() + 1 ==
             m_tree_model->rowCount()  //Последний элемент в списке
  };
  ...
  PipelineBuilder()
      .AddHandler([&builder, &msg_factories = m_message_factories](bool& _) {
      ...
      .AddHandler([this, &arg_tuple](bool& successfully_inserted) {
        ...
        if (append) {
          ...
          if (auto it = insert_department_helper(
                  std::move(std::get<Department>(department_holder)),
                  *m_tree_model->GetDepartmentNameRef(m_tree_model->GetNextItem(
      ...
      .AddHandler([this, &arg_tuple](bool& successfully_inserted) {
        ...
        auto [value, result_type]{m_tasks->tree_model.Process(
      ...
      .AddHandler([&task_manager = m_tasks->modify_xml](bool& _) {
...
bool CompanyManagerUI::remove_department(const DepartmentViewInfo& view_info) {
  ...
  PipelineBuilder()
      .AddHandler([this, &arg_tuple](bool& _0) {
        ...
        auto [_1, result_type]{m_tasks->tree_model.Process(
      ...
      .AddHandler([this, &arg_tuple](bool& successfully_removed) {
        ...
        auto [_1, result_type]{m_tasks->modify_xml.Process(
            command::xml_wrapper::RemoveDepartment::make_instance(
                *m_company_manager, department_name))};
        if (result_type != task::ResultType::Success) {
          handle_internal_fatal_error(m_tasks->modify_xml);
        } else {
...
bool CompanyManagerUI::new_employee(const DepartmentViewInfo& view_info) {
  ...
  PipelineBuilder()
      ...
      .AddHandler([this, &arg_tuple](bool& _0) {
        ...
        auto [value, result_type]{m_tasks->modify_xml.Process(
            command::xml_wrapper::InsertEmployee::make_instance(
                *m_company_manager, extract_department_name(view_info),
      ...
      .AddHandler([&task_manager = m_tasks->modify_xml](
  ...
  if (successfully_inserted) {
    m_item_viewers.department->UpdateStats();  //Т.к. операция добавления
...
bool CompanyManagerUI::remove_employee(
  ...
  PipelineBuilder()  //следующего за удалённым!
      ...
      .AddHandler([this, &personal_data](bool& successfully_removed) {
        auto [_, result_type]{m_tasks->modify_xml.Process(
            command::xml_wrapper::RemoveEmployee::make_instance(
                *m_company_manager, personal_data))};
        if (result_type != task::ResultType::Success) {
          handle_internal_fatal_error(m_tasks->modify_xml);
        } else {
          successfully_removed = true;
      ...
      .AddHandler([this, &arg_tuple](bool& successfully_changed) {
        ...
        auto [value, result_type]{m_tasks->modify_xml.Process(
            command::xml_wrapper::ChangeEmployeeSurname::make_instance(
                *m_company_manager, extract_employee_personal_data(view_info),
...
void CompanyManagerUI::connect_menu_bar() {
  ...
  connect(m_gui->save_as_btn, &QAction::triggered, this,
          &CompanyManagerUI::save_as);
  connect(m_gui->close_btn, &QAction::triggered, this,
  ...
  connect(m_gui->about_program, &QAction::triggered, this,
...
void CompanyManagerUI::connect_item_viewers() {
  connect(m_gui->new_department_btn, &QPushButton::clicked, this,
          &CompanyManagerUI::new_department);
  connect(m_item_viewers.department, &DepartmentView::name_changed, this,
          &CompanyManagerUI::rename_department);
  connect(m_item_viewers.department, &DepartmentView::erased, this,
          &CompanyManagerUI::remove_department);
  connect(m_item_viewers.department, &DepartmentView::employee_added, this,
          &CompanyManagerUI::new_employee);
  connect(m_item_viewers.employee, &EmployeeView::surname_changed, this,
          &CompanyManagerUI::change_employee_surname);
  connect(m_item_viewers.employee, &EmployeeView::name_changed, this,
          &CompanyManagerUI::change_employee_name);
  connect(m_item_viewers.employee, &EmployeeView::middle_name_changed, this,
          &CompanyManagerUI::change_employee_middle_name);
  connect(m_item_viewers.employee, &EmployeeView::function_changed, this,
          &CompanyManagerUI::change_employee_function);
  connect(m_item_viewers.employee, &EmployeeView::salary_updated, this,
          &CompanyManagerUI::update_employee_salary);
  connect(m_item_viewers.employee, &EmployeeView::erased, this,
...
void CompanyManagerUI::setup_viewers() {
  ...
  m_gui->stacked_viewer->setCurrentWidget(
      m_item_viewers.dummy);  //Устанавливаем пустое окно просмотра
  show_viewers(true);
}
...
void CompanyManagerUI::update_department_viewer(
  ...
  m_item_viewers.department->SetDepartment(DepartmentView::view_info{
      selected_item,
      std::get<const Department*>(m_tree_model->GetItemNode(selected_item))});
}

void CompanyManagerUI::update_employee_viewer(
    const QModelIndex& selected_item) {
  m_item_viewers.employee->SetEmployee(EmployeeView::view_info{
      selected_item,
      std::tuple{
...
void CompanyManagerUI::reset_helper() {  //Сброк при закрытии документа
  m_tasks->service.Process(
      command::file_io::Reset::make_instance(*m_company_manager));
  m_tasks->service
...
bool CompanyManagerUI::is_loaded() const {
  return std::any_cast<bool>(
      m_tasks->service
          .Process(
              command::file_io::CheckLoaded::make_instance(*m_company_manager))
          .value);
}
...
bool CompanyManagerUI::is_saved() const {
  return std::any_cast<bool>(
      m_tasks->service
          .Process(
              command::file_io::CheckSaved::make_instance(*m_company_manager))
          .value);
}

void CompanyManagerUI::set_file_path(const QString& path) {
  m_tasks->service.Process(command::file_io::SetPath::make_instance(
      *m_company_manager, path.toStdString()));
}
...
std::optional<worker::file_operation::Result> CompanyManagerUI::load_helper() {
  auto [value, result_type]{m_tasks->service.Process(
...
CompanyManagerUI::add_department_helper(Department&& department) {
  auto [value, result_type]{m_tasks->modify_xml.Process(
      command::xml_wrapper::AddDepartment::make_instance(
          *m_company_manager, std::move(department)))};
  if (result_type != task::ResultType::Success) {
    handle_internal_fatal_error(m_tasks->modify_xml);
    return std::nullopt;
  }
...
CompanyManagerUI::insert_department_helper(Department&& department,
  ...
  auto [value, result_type]{m_tasks->modify_xml.Process(
      command::xml_wrapper::InsertDepartment::make_instance(
          *m_company_manager, before, std::move(department)))};
  if (result_type != task::ResultType::Success) {
    handle_internal_fatal_error(m_tasks->modify_xml);
    return std::nullopt;
  }
      ...
      .AddHandler([this, &arg_tuple](bool& successfully_renamed) {
        ...
        auto [value, result_type]{m_tasks->modify_xml.Process(
            command::xml_wrapper::RenameDepartment::make_instance(
                *m_company_manager, old_name, new_name.toStdString()))};
        if (result_type != task::ResultType::Success) {
          handle_internal_fatal_error(m_tasks->modify_xml);
          return false;
        }
      ...
      .AddHandler([this, &arg_tuple](bool& successfully_renamed) {
        ...
        auto [value, result_type]{m_tasks->tree_model.Process(
            command::tree_model::RenameDepartment::make_instance(
                *m_tree_model, view_info.index.row(), new_name))};
        if (result_type != task::ResultType::Success) {
          handle_internal_fatal_error(m_tasks->tree_model);
        } else {
...
bool CompanyManagerUI::rename_employee_in_tree_model(
  ...
  auto [new_q_idx, result_type]{m_tasks->tree_model.Process(
