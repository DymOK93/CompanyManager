...
namespace xml {
Writer::Writer(ostream& output) noexcept : m_output(addressof(output)) {}
...
void Writer::print_indents(optional<size_t> indents_count) {
  if (m_indent && indents_count) {
    m_indent->WriteIndents(
        get_stream(),
        *indents_count);  //В классе хранится начальное число отступов
...
void Writer::print_service_node_header(const Node& node) {
  get_stream() << '<' << read_service_block(node.AsService().first)
               << node.GetName();
...
string Writer::read_service_block(service_block_t block) {
  std::string buffer(  //Копируем символы в буферную строку
      reinterpret_cast<const char*>(addressof(block)), sizeof(service_block_t));
...
void Writer::serialize_container(const Node& node,
                                 std::optional<size_t> indents_count) {
  serialize_container_helper(node.AsContainer(), indents_count);
}

void Writer::serialize_container_helper(const container_t& container,
                                        optional<size_t> indents_count) {
  for (const auto& node : container) {
    serialize_node(*node, indents_count);
  }
}

optional<size_t> Writer::increment_indents_count(
    optional<size_t> indents_count) {
