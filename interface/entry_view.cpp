...
DepartmentView::DepartmentView(QWidget* parent) : MyBase(parent) {
  ...
  connect(m_gui->name_lnedit, &QLineEdit::returnPressed, this,
          &DepartmentView::name_changed_transmitter);
  connect(m_gui->erase_department_btn, &QPushButton::clicked, this,
          &DepartmentView::erased_transmitter);
  connect(m_gui->add_employee_btn, &QPushButton::clicked, this,
          &DepartmentView::employee_added_transmitter);
  m_gui->employee_count_lnedit->setReadOnly(
...
void DepartmentView::RestoreName() {
  m_gui->name_lnedit->setText(QString::fromStdString(
      get_item<const wrapper::Department*>()->GetName()));
}

void DepartmentView::UpdateStats() {
  auto& department_it{
      get_item<const wrapper::Department*>()};  //«апрос статистики
                                                //подразделени€
...
void DepartmentView::set_read_only_mode(bool on) {
  m_gui->name_lnedit->setReadOnly(
      on);  //«апрет или разрешение редактировани€ наименовани€
}
