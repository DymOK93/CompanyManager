...
namespace wrapper {
XmlWrapper::XmlWrapper(Node* node_ptr) : m_node(node_ptr) {}

XmlWrapper::XmlWrapper(node_holder ready_node) : m_node(move(ready_node)) {}
...
XmlWrapper& XmlWrapper::OwnFrom(XmlWrapper& source) {
  ...
  throw_if_different_wrapper_types(
      get_type(),
      source.get_type());  //В take_dependencies() будет выполнен dynamic_cast
...
node_holder XmlWrapper::BuildXmlTree() {
  Synchronize();  //Предварительная синхронизация обязательна во избежание утери
                  //изменений
  if (IsObserver()) {
...
void XmlWrapper::throw_if_another_node_type(const Node& node,
                                            Node::Type expected) {
  if (auto type = node.GetType(); type != expected) {
    throw runtime_error(
        "Invalid XML node type: " + string(Node::TypeAsString(expected)) +
        " expected, but " + string(Node::TypeAsString(type)) + " found");
  }
}
...
Employee::Employee(xml::Node* node_ptr)
    : XmlWrapper(node_ptr), m_properties(collect_properties(*node_ptr)) {}
...
FullNameRef Employee::GetFullName() const {
  return FullNameRef{*m_properties.at("surname"), *m_properties.at("name"),
...
Employee::properties_view_t Employee::collect_properties(Node& node) {
  throw_if_another_node_type(
      node, Node::Type::Tree);  // XML-узел должен хранить дочерние узлы

  auto& raw_properties{node.AsContainer()};
  properties_view_t properties;
  for (auto& employee_holder : raw_properties) {
    properties.emplace(employee_holder->GetName(),
                       addressof(employee_holder->AsText()));
  }
  return properties;
}
...
RenameResult Department::employee_rename_helper(const FullNameRef& old_name,
                                                string&& value,
                                                FullNameField field) {
  FullNameRef new_full_name(old_name);
  switch (field) {  //Для поиска дубликатов
    case FullNameField::Surname:
      new_full_name.surname = value;
      break;
    case FullNameField::Name:
      new_full_name.name = value;
      break;
    case FullNameField::MiddleName:
      new_full_name.middle_name = value;
      break;
  }

  if (m_workgroup.count(
          new_full_name)) {  //Смена имени требует выполнения "тяжелой" операции
                             //- извлечения из workgroup
    return new_full_name == old_name
               ? RenameResult::NothingChanged
               : RenameResult::IsDuplicate;  //Найдены дубликаты или имя не
                                             //изменилось
  }

  auto employee_it{
      m_workgroup.find(old_name)};  // todo: изменение позиции в кэше
  if (employee_it == m_workgroup.end()) {
    throw_non_existent_key("Employee with full name " +
                           FullNameRefAsString(old_name));
  }
  register_rename(old_name,
                  new_full_name);  //Обновление позиции элемента в кэше
  ...
  switch (field) {  //Обновление поля
    case FullNameField::Surname:
      node.mapped().SetSurname(move(value));
      break;
    case FullNameField::Name:
      node.mapped().SetName(move(value));
      break;
    case FullNameField::MiddleName:
      node.mapped().SetMiddleName(move(value));
      break;
  };
...
void Department::recalc_summary_salary_after_recruitment(
    size_t new_employee_salary) {
  m_summary_salary += new_employee_salary;
}

void Department::recalc_summary_salary_before_dismissal(
    size_t former_employee_salary) {
...
Department& Department::Synchronize() {
  ...
  if (!staff) {
    ...
    cont.push_back(xml::DocumentNodeBuilder()
                       .SetName("employments")
  ...
  synchronize_helper<Employee>(
      addressof(staff->AsContainer()), m_workgroup.begin(), m_workgroup.end(),
      m_workgroup.size(),
      [](workgroup_t::value_type& name_and_employee)
          -> Employee& {  //Без явного указания, что возвращается ссылка,
        return name_and_employee.second;  //будет попытка копирования (т.к.
                                          //возвращается поле аргумента ф-ции)
      });
  return *this;
}

bool operator<(const FullNameRef& left, const FullNameRef& right) {
  return tie(left.surname.get(), left.name.get(), left.middle_name.get()) <
...
size_t FullNameHasher::operator()(const FullNameRef& name) const noexcept {
  ...
  return hash<string>()(name.surname) * coef * coef +
         hash<string>()(name.surname) * coef + hash<string>()(name.surname);
}

string FullNameRefAsString(const FullNameRef& full_name) {
  string full_name_str;
  full_name_str.reserve(
      full_name.surname.get().size() + full_name.name.get().size() +
      full_name.middle_name.get().size() + 2  //Пробельные символы
  );
...
xml::Node* Department::try_get_staff(
    xml::Node&
        node) {  //Попытка обращения к XML-узлу <employments>...</employments>
  auto& raw_info{node.AsContainer()};
  auto staff_it{
      find_if(raw_info.begin(), raw_info.end(), [](const node_holder& node) {
        return node->GetName() == "employments";
      })};
  return staff_it == raw_info.end() ? nullptr : staff_it->get();
}

Department::salary_t Department::calc_summary_salary(
    const workgroup_t& workgroup) {
...
RenameResult Department::ChangeEmployeeSurname(const FullNameRef& old_name,
                                               std::string new_surname) {
  return employee_rename_helper(old_name, move(new_surname),
                                FullNameField::Surname);
}

RenameResult Department::ChangeEmployeeName(const FullNameRef& old_name,
                                            std::string new_name) {
  return employee_rename_helper(old_name, move(new_name), FullNameField::Name);
}

RenameResult Department::ChangeEmployeeMiddleName(const FullNameRef& old_name,
                                                  std::string new_middle_name) {
  return employee_rename_helper(old_name, move(new_middle_name),
                                FullNameField::MiddleName);
}

Department& Department::UpdateEmployeeSalary(const FullNameRef& full_name,
                                             size_t new_salary) {
...
double Department::AverageSalary() const noexcept {
  return EmployeeCount()
             ? static_cast<double>(m_summary_salary) / EmployeeCount()
             : 0;
}
...
Department::employee_it Department::InsertEmployee(Employee&& employee) {
  auto [it,
        success]{m_workgroup.emplace(employee.GetFullName(), move(employee))};
  if (success) {
...
Department::employee_it Department::EraseEmployee(employee_it employee) {
  ...
  register_erase(employee->second.GetFullName(),
                 employee == prev(m_workgroup.end()));
...
Company& Company::take_dependencies(XmlWrapper& other) {
  auto& other_company{
      static_cast<Company&>(other)};  //Допустимо, т.к. в случае несовпадения
                                      //типов исключение выбросит OwnFrom()
...
Company& Company::Synchronize() {
  synchronize_helper<Department>(  //Тип элементов диапазона
      addressof(get_node().AsContainer()), m_subdivision.begin(),
      m_subdivision.end(), m_subdivision.size(),
...
RenameResult Company::RenameDepartment(const string& department,
                                       string new_name) {
  if (m_subdivision_map.count(
          new_name)) {  //Операции извлечения и вставки - долгие. Эффективнее
                        //предварительно сравнить строки
    return department == new_name ? RenameResult::NothingChanged
                                  : RenameResult::IsDuplicate;
  }
...
void Company::insert_to_subdivision_map(subdivision_map_t* map,
                                        department_it it) {
  map->emplace(it->GetName().get(), it);
}

Company::department_it Company::erase_from_subdivision(
...
Department Company::extract_from_subdivision(department_it department) {
  register_erase(department->GetName().get(),
...
Company::department_it Company::AddDepartment(Department&& new_department) {
  if (auto already_exists_it =
          m_subdivision_map.find(new_department.GetName().get());
...
Company::department_it Company::InsertDepartment(const std::string& before,
...
Company::department_it Company::InsertDepartment(department_it before,
                                                 Department&& new_department) {
