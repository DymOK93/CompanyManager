...
class XmlWrapper {
 public:
  enum class Mode { View, Own };
  enum class Type { Employee, Department, Company };
  static constexpr std::string_view ModeAsString(Mode mode) {
    return mode_as_string[static_cast<size_t>(mode)];
  }

 private:
  static constexpr std::array<std::string_view, 4> mode_as_string{"view",
                                                                  "own"};
 ...
 public:
  ...
  virtual XmlWrapper& OwnFrom(
      XmlWrapper& source);  //Создает узел, перемещает в него данные из source и
                            //переключается в режим владения
  XmlWrapper& CopyFrom(
      const XmlWrapper&
          source);  //Копирует содержимое из узла, на который ссылается source

  virtual XmlWrapper& Synchronize() = 0;  //Синхронизирует дочерние элементы без
                                          //перемещения владеющего узла.
  virtual XmlWrapper& Reset() = 0;  //Сбрасывает указатель на узел
  virtual xml::node_holder BuildXmlTree();
  static xml::node_holder BuildXmlTree(
      XmlWrapper& wrapper);  //Возвращает владеющий узел и переключается в режим
                             //наблюдения

  xml::allocator_holder GetAllocator()
      const noexcept;  //Позволяет получить аллокато из прикрепленного узла
 protected:
  ...
  static void throw_if_another_node_type(const xml::Node&,
                                         xml::Node::Type expected);
  static void throw_if_different_wrapper_types(Type left, Type right);
  static void throw_non_existent_key(std::string key);

  virtual Type get_type()
      const noexcept = 0;  //Запрашивает тип текущего объекта
  virtual XmlWrapper& update_dependencies() = 0;  //Обновляют служебные данные
                                                  //после вызова OwnFrom()
  virtual XmlWrapper& take_dependencies(
      XmlWrapper& other) = 0;  //Передает служебные данные в другой объект

  xml::Node& get_node();
  const xml::Node& get_node() const;

 protected:
  std::variant<xml::Node*,       //Режим наблюдения
...
/***********************************************************************************************************************
Использовать string_view небезопасно, поскольку при установке нового значения
поля строка const char*, на которую ссылается string_view, будет безвозвратно
утеряна. Чтобы не вынуждать хранящие FullName внешние объекты делать полную
копию, целесообразно применить StringRef, т.к. сами объекты строк не будут
инвалидированы до конца срока жизни хранящих их XML-узлов. (Поля данных
работника - дочерние узлы Employee-node, имя компании - элемент перемещаемого
контейнера внутри Department-node). Начиная с С++17, Стандартом гарантируется,
что string_ref - TriviallyCopyable
************************************************************************************************************************/

using string_ref = std::reference_wrapper<const std::string>;

struct FullNameRef {
  string_ref surname, name, middle_name;
};
...
class Employee : public XmlWrapper {
 ...
 public:
  ...
  Employee& Synchronize() override;  //Метод-заглушка: синхронизация выполняется
                                     //непосредственно при внесении изменений
  ...
  Employee& SetSurname(
      std::string new_surname);  //Т.к. смена полей ФИО требует смены позиции в
                                 //подразделении,
  Employee& SetName(std::string new_name);  //она должна выполняться только
                                            //через интерфейс последнего!
  Employee& SetMiddleName(
      std::string new_middle_name);  //(если работник не извлечён)
  Employee& SetFunction(std::string new_function);
  Employee& SetSalary(
      size_t new_salary);  //Для корректного сбора статистики смена З/П должна
                           //производиться через интерфейс подразделения
...
template <class CachedTy,
          class Hasher = std::hash<CachedTy>>  // CachedTy must be easy-copyable
class XmlContainerWrapper : public XmlWrapper {
 ...
 protected:
  void register_insert(const CachedTy& value, bool insert_back) {
    ...
    if (insert_back) {
      if (m_state.erased_back_count > 0) {
        ...
        force_rebuild();  //Во избежании ситуации с удалением и вставкой
                          //одинакового кол-ва разных элементов
      } else {
  ...
  void register_erase(const CachedTy& value, bool erase_back) {
    if (erase_back) {
      m_state.inserted_back_count > 0 ? --m_state.inserted_back_count
                                      : ++m_state.erased_back_count;
      m_inserted.erase(value);
    } else {
      if (auto it = m_inserted.find(value); it == m_inserted.end()) {
        force_rebuild();
      } else {
        m_inserted.erase(it);
      }
  ...
  void register_rename(const CachedTy& old_value, const CachedTy& new_value) {
    if (auto it = m_inserted.find(old_value); it != m_inserted.end()) {
  ...
  bool contains(const CachedTy& value) {
    return static_cast<bool>(m_inserted.count(
        value));  //До C++20 у unordered_set нет метода contains()
  }

  void force_rebuild() { m_state.force_rebuild = true; }

  template <class WrapperTy,
            class InputIt,
            class Extractor>  // Extractor не должены быть тяжелым, т.к. будет
                              // скопирован
                              void
                              synchronize_helper(  // WrapperTy - тип дочерних
                                                   // элементов, хранящихся в
                                  ...
                                  size_t range_length,
                                  Extractor extractor) {
    static_assert(std::is_base_of_v<XmlWrapper, WrapperTy>,
                  "WrapperTy must be derived from XMLWrapper");
    if (m_state.force_rebuild) {
      ...
      sync_and_update_size<WrapperTy>(
          dst, first, last, range_length,
          extractor);  //однако состояние более нижних уровней неизвестно
    }  //если dst->size() == range_length (нет вновь вставленных и удаленных
       //узлов),
  ...
  void rebuild_tree(xml::container_t* dst,
                    InputIt first,
                    InputIt last,
                    size_t range_length,
                    Extractor extractor) {
    ...
    collect_children<WrapperTy>(
        std::addressof(
            buffer),  //Перестроенные ветви собираются в буферное хранилище
        first, last, range_length, extractor);
    *dst = std::move(buffer);
  }

  template <class WrapperTy, class InputIt, class Extractor>
  void collect_children(xml::container_t* dst,
                        InputIt first,
                        InputIt last,
                        size_t range_length,
                        Extractor extractor) {
    dst->reserve(dst->size() +
                 range_length);  //Если dst не пуст, нужно вызвать reserve(),
                                 //т.к. элементы добавляются в конец
    for (; first != last; ++first) {
      auto& wrapper{extractor(*first)};
      if (wrapper.IsObserver()) {  //Если узел - наблюдатель, отобрать владение
        wrapper = MoveFrom<WrapperTy>(
            wrapper);  //у находящегося в таинственной неизвестности unique_ptr
  ...
  template <class WrapperTy,
            class InputIt,
            class Extractor>  //Перестраиваются только вновь вставленные
                              //элементы
                              void sync_and_update_size(xml::container_t* dst,
                                                        InputIt first,
                                                        InputIt last,
                                                        size_t range_length,
                                                        Extractor extractor) {
    first = synchronize_range(
        first, std::min(dst->size(), range_length),
        extractor);  //Синхронизация ветвей, не требующих сборки XML-поддерева
    if (range_length <= dst->size()) {
      ...
      collect_children<WrapperTy>(
          dst, first, last,
          range_length -
              dst->size(),  // first уже сдвинут на dst.size() позиций
          extractor);
    }
  }
  ...
  InputIt synchronize_range(
      InputIt range_begin,
      size_t range_length,  //Передавать last не имеет смысла - важны только
                            //начало диапазона и его длина
      Extractor&& extractor) {
    while (range_length--) {
...
enum class RenameResult {  //Результат попытки сменить имя подразделения или
                           //сотруднкиа
  Success,
  IsDuplicate,
  NothingChanged
...
class Department : public XmlContainerWrapper<FullNameRef, FullNameHasher> {
 public:
  ...
  using employee_it =
      workgroup_t::iterator;  //Гарантируется, что итераторы не будут
                              //инвалидированы при любых операциях
  using employee_view_it =
      workgroup_t::const_iterator;  //с Employee из конкреного Department, кроме
                                    //вызовов Extract.../Erase...
 ...
 public:
  ...
  RenameResult ChangeEmployeeSurname(
      const FullNameRef& old_name,
      std::string new_surname);  //Т.к. смена полей ФИО требует смены позиции в
                                 //m_workgroup,
  RenameResult ChangeEmployeeName(
      const FullNameRef& old_name,
      std::string new_name);  //оно должно выполняться только через интерфейс
                              //подразделения!
  RenameResult ChangeEmployeeMiddleName(const FullNameRef& old_name,
                                        std::string new_middle_name);

  Department& UpdateEmployeeSalary(
      const FullNameRef& full_name,
      size_t new_salary);  //Для сбора правильной статистики изменение зарплаты
                           //выполняется
 ...
 protected:
  ...
  RenameResult employee_rename_helper(
      const FullNameRef& old_name,
      std::string&& value,
      FullNameField
          field);  //Во избежание дублирования кода операции извлечения
  ...
  static xml::Node* try_get_staff(
      xml::Node&);  //Получение указателя на узел <employments> внутри
                    //<department>
 ...
 protected:
  ...
  salary_t m_summary_salary{0};  //Лучше хранить суммарную и вычислять среднюю,
                                 //чем хранить среднюю и накапливать погрешность
};
...
class Company : public XmlContainerWrapper<std::string_view> {
 ...
 public:
  ...
  RenameResult RenameDepartment(const std::string& department,
                                std::string new_name);
  ...
  department_it InsertDepartment(const std::string& before,
                                 Department&& new_department);
  department_it InsertDepartment(department_it before,
                                 Department&& new_department);
 ...
 protected:
  ...
  department_it erase_from_subdivision(
      department_it department);  //Удаляет из m_subdivision без проверки
                                  //наличия в m_subdivision_map
  Department extract_from_subdivision(
      department_it department);  //Извлекает из m_subdivision без проверки
                                  //наличия в m_subdivision_map

  static void insert_to_subdivision_map(subdivision_map_t* map,
                                        department_it it);
