...
CompanyTreeItem::CompanyTreeItem(wrapper::XmlWrapper::Type type,
                                 const NodeInfo& node_info,
                                 CompanyTreeItem* parent)
    : m_node_type(type), m_node_info(std::move(node_info)), m_parent(parent) {}
...
CompanyTreeItem* CompanyTreeItem::child(size_t idx) {
  return idx < m_child_items.size() ? m_child_items[idx].get() : nullptr;
}
...
bool CompanyTreeItem::insertChild(tree_item_holder&& child, size_t position) {
  ...
  m_child_items.insert(m_child_items.begin() + position, std::move(child));
  return true;
}
...
CompanyTreeItem::tree_item_holder& CompanyTreeItem::operator[](
    size_t idx) noexcept {
  return m_child_items[idx];
}

size_t CompanyTreeItem::find_child(const children_list& children,
  ...
  if (it == children.end()) {
    throw std::logic_error(
        "The parent has no information about the child");  //Защита от узлов с
                                                           //некорректным parent
  }
...
const CompanyTreeItem::wrapper_node_ptr& CompanyTreeItem::GetWrapperNode()
    const noexcept {
  return m_node_info.wrapper_node;
}

CompanyTreeItem& CompanyTreeItem::SetWrapperNode(
    wrapper_node_ptr node_ptr) noexcept {
...
size_t CompanyTreeItem::Find(const QString& request) {
  auto it{std::find_if(m_child_items.begin(), m_child_items.end(),
...
size_t CompanyTreeItem::UpperBound(const QString& request) {
  auto it{std::upper_bound(
      m_child_items.begin(), m_child_items.end(), request,
      [](const QString& value, const tree_item_holder& item) {
        return QString::compare(value, item->GetName(), Qt::CaseInsensitive) <
...
std::optional<size_t> CompanyTreeItem::MoveChild(size_t from, size_t to) {
  if (from == to  //Проверка во избежание ненужного выполнения длительной - O(N)
                  //- операции сдвига элементов
      || from >= m_child_items.size() || to > m_child_items.size()) {
    return std::nullopt;
  }
