...
namespace command {
	namespace tree_model {
		...
		std::any RemoveEmployee::Execute() {									
			...
			if (!m_employee_name) {				
				...
				m_employee_name = std::get<const Employee*>(					//На момент выполнения Execute() элемент модели и указатель еще валидны, 
						MyBase::get_target().GetItemNode(employee_q_idx)		//поскольку удаление из дерева данных выполняется позже
					)->GetFullName();
			}
			return MyBase::get_target().RemoveEmployeeItem(						
				department_q_idx,
		...
		std::any RemoveEmployee::Cancel() {										//Для Remove-операций восстановление в дереве данных должно производиться раньше
			auto& tree_model{ MyBase::get_target() };							//восстановления в модели, чтобы можно было извлечь необходимые сведения
