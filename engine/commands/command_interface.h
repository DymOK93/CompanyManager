...
enum class Purpose {  //Объединение команд различных типов в более крупные
                      //систематические единицы для регулирования порядка
                      //выполнения и отмены
  FileIO = 7,
  Insert = 12,
  Remove = 16,
...
class ICommand {
 ...
 protected:
  TargetTy& get_target() const noexcept { return *m_target; }
  static std::any make_default_value() noexcept { return std::any{}; }

 private:
  static constexpr std::array<std::underlying_type_t<Purpose>, 4>
      command_purpose{static_cast<purpose_t>(Purpose::FileIO),
...
class AllocatedCommand : public ObjectPool<ICommand<TargetTy>, ConcreteCommand>,
                         public ICommand<TargetTy> {
 public:
 ...
 public:
  ...
  static command_holder make_instance(
      TargetTy& target) {  //Для классов команд без дополнительных аргументов
                           //конструктора
