...
class NodeBuilderBase : public BuilderBase<NodeBuilderBase<ConcreteBuilder>> {
 ...
 public:
  ConcreteBuilder& SetAllocator(
      allocator_holder alloc) {  //Для возврата из BuilderBase ссылки на
                                 //наследующие NodeBuilderBase классы
  ...
  node_holder Assemble() {
    ...
    return make_node_holder(
        alloc, Node::Header{move(m_name), move(m_attrs)},
        alloc  //Перемещать нельзя - порядок вычисления аргументов функции не
  ...
  node_holder Build() {
    ...
    return make_node_holder(alloc, Node::Header{m_name, m_attrs}, alloc);
  }

 protected:
  template <class... Types>
  static node_holder make_node_holder(allocator_holder target_alloc,
                                      Types&&... args) {
    node_holder holder(target_alloc->allocate(1), make_deleter(target_alloc));
    std::allocator_traits<allocator_t>::construct(*target_alloc, holder.get(),
                                                  std::forward<Types>(args)...);
    return holder;
  }

  static std::function<void(Node*)> make_deleter(
      allocator_holder target_alloc) {
...
class NodeBuilder : public NodeBuilderBase<NodeBuilder> {
 ...
 public:
  static node_holder Duplicate(const Node& source) {  //Копирование ветви дерева
    ...
    node_holder deep_copy{make_node_holder(
        source.GetAllocator(),
        Node::Header{source.GetName(),
  ...
  static node_holder Take(
      Node&& source) {  //Перемещение узла с принадлежащими ему ветвями дерева
    allocator_holder alloc{source.GetAllocator()};
    return make_node_holder(alloc, std::move(source));
  }
  ...
  static void duplicate_value(Node& target, const Node& source) {
    switch (source.GetType()) {
      case Node::Type::Service:
        target.SetService(source.AsService());
        break;
      case Node::Type::Element:
        target.SetText(source.AsText());
        break;
      case Node::Type::Tree:
        target.SetContainer(duplicate_children(source.AsContainer()));
        break;
      default:
        break;
    }
...
class ServiceNodeBuilder : public NodeBuilderBase<ServiceNodeBuilder> {
 ...
 public:
  ServiceNodeBuilder& AddServiceSymbols(service_block_t front,
