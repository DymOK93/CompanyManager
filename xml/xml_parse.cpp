...
namespace xml {
Reader::Reader(istream& input) noexcept : m_input(addressof(input)) {}

Document Reader::Load(allocator_holder external_alloc) {
  m_tree_allocator =
      move(external_alloc);  //Может использоваться внешнийс аллокатор
  return m_builder
...
node_holder Reader::load_node() {
  ...
  text_t name{load_line([](char ch) { return ch != '>' && !isspace(ch); })};

  node_holder node(NodeBuilder()
                       .SetName(move(name))
...
void Reader::load_node_value(Node& node,
                             optional<int64_t> first_service_block) {
  auto second_servie_block{load_service_block()};
  load_text('>');
  bool new_line{left_strip()};

  if (first_service_block) {
    node.SetService(make_pair(*first_service_block,
                              second_servie_block
                                  ? optional<int64_t>(*second_servie_block)
                                  : nullopt));
  } else if (peek_next() == '<' &&
             new_line) {  //</...> без перевода строки означает конец пустого
                          //текстового блока
...
optional<service_block_t> Reader::load_service_block() {
  string buffer{load_line([](byte symbol) {
  ...
  buffer.resize(
      ...
      static_cast<char>(
          0));  //Дополняем нулям для корретного считывания в дальнейшем
  buffer.copy(reinterpret_cast<char*>(addressof(result)),
              sizeof(service_block_t), 0);  //Копируем содержимое строки
  return result;
}
