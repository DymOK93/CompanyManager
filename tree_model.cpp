...
CompanyTreeModel& CompanyTreeModel::SetCompany(const CompanyManager& cm) {
  beginResetModel();  //Уведомляем все привязанные view'ы об инвалидации даных
                      //модели
...
int CompanyTreeModel::rowCount(const QModelIndex& parent) const {
  ...
  return m_root_item ?  //После сброса Reset() внешние объекты могут запрашивать
                        //кол-во дочерних узлов для перерисовки
             static_cast<int>(m_root_item->childCount())
                     : 0;
}

int CompanyTreeModel::columnCount(const QModelIndex& parent) const {
  (void)parent;  //Неиспользуемый параметр
  return m_root_item ? static_cast<int>(m_root_item->columnCount()) : 1;
}
...
QModelIndex CompanyTreeModel::parent(const QModelIndex& index) const {
  ...
  CompanyTreeItem* parent_item{child_item ? child_item->parent() : nullptr};

  if (!parent_item || parent_item == m_root_item.get())
    return QModelIndex();

  return createIndex(static_cast<int>(parent_item->childNumber()), 0,
                     parent_item);
}
...
QModelIndex CompanyTreeModel::EmployeeIndex(const QModelIndex& department,
                                            size_t row) const {
...
QModelIndex CompanyTreeModel::index(int row,
                                    int column,
...
QVariant CompanyTreeModel::data(const QModelIndex& index,
...
std::optional<CompanyTreeModel::ItemType> CompanyTreeModel::GetItemType(
...
std::optional<QString> CompanyTreeModel::GetItemName(
    const QModelIndex& index) const noexcept {
  auto* item{get_item(index)};
  if (!item || item == m_root_item.get()) {  //Имя компании в текущей модели не
                                             //предусмотрено, а её data_storage
    return std::nullopt;  //имеет размер 1!
  }                       //Проверка во избежание UB
  return item->GetName();
}

std::optional<QString> CompanyTreeModel::ExchangeItemName(
    const QModelIndex& index,
    QString new_name) noexcept {
  auto* item{get_item(index)};
...
CompanyTreeModel::wrapper_node_ptr CompanyTreeModel::GetItemNode(
    const QModelIndex& index) {
  auto* item{get_item(index)};
  if (!item) {
    return {};
...
std::optional<wrapper::string_ref> CompanyTreeModel::GetDepartmentNameRef(
    const QModelIndex& index) {
  auto* item{get_item(index)};
  if (!item) {
    return std::nullopt;
  ...
  switch (item->GetType()) {
    case ItemType::Employee:
      return item->GetParentWrapperName();
    case ItemType::Department:
      return std::get<const wrapper::Department*>(item->GetWrapperNode())
          ->GetName();
    default:
      return std::nullopt;
  };
...
QModelIndex CompanyTreeModel::GetPrevItem(const QModelIndex& old_index) {
  ...
  return old_index.row() == 0 ? QModelIndex()
                              : index(old_index.row() - 1, 0, parent_idx);
}
...
QModelIndex CompanyTreeModel::GetNextItem(const QModelIndex& old_index) {
  ...
  QModelIndex parent_idx(
      parent(old_index));  //Родительский (возможно, корневой) индекс
...
std::optional<size_t> CompanyTreeModel::FindChild(const QString& request,
...
std::optional<size_t> CompanyTreeModel::UpperBoundChild(
...
std::optional<size_t> CompanyTreeModel::MoveItem(size_t row_from,
                                                 size_t row_to,
                                                 const QModelIndex& parent) {
  auto item{get_item(parent)};
  ...
  if (beginMoveRows(  //Все необходимые проверки возложены на beginMoveRows()
          parent, static_cast<int>(row_from),
          static_cast<int>(row_from),  // source
          parent,
...
QModelIndex CompanyTreeModel::InsertDepartmentItem(const Department& source,
                                                   size_t pos) {
  if (!m_root_item || pos > m_root_item->childCount()) {
    return QModelIndex();
  }
  beginInsertRows(QModelIndex(), static_cast<int>(pos),
                  static_cast<int>(pos));  // root index
  bool success{m_root_item->insertChild(
      build_department_tree(source, m_root_item.get()), pos)};
  endInsertRows();
  if (!success) {
    return QModelIndex();
  }
...
bool CompanyTreeModel::RemoveDepartmentItem(size_t pos) {
  ...
  beginRemoveRows(QModelIndex(), static_cast<int>(pos),
                  static_cast<int>(pos));  // root index
...
CompanyTreeModel::Memento CompanyTreeModel::DumpDepartmentItem(size_t pos) {
  ...
  Memento::Dump dump{pos, std::nullopt, std::move(company[pos])};
...
bool CompanyTreeModel::RestoreDepartmentItem(Memento&& item,
                                             const Department* new_node) {
  Memento::Dump department_dump(item.Extract());
  if (new_node) {
    department_dump.branch->SetWrapperNode(new_node);
  }
  beginInsertRows(QModelIndex(),  // root index
                  static_cast<int>(department_dump.pos),
                  static_cast<int>(department_dump.pos));
  bool success{m_root_item->insertChild(std::move(department_dump.branch),
                                        department_dump.pos)};
  endInsertRows();
  return success;
}

QModelIndex CompanyTreeModel::InsertEmployeeItem(const Employee& source,
                                                 const QModelIndex& department,
                                                 size_t pos) {
  auto* item{get_item(department)};
  if (!item || item->GetType() != ItemType::Department ||
      pos > item->childCount()) {
    return QModelIndex();
  }
  tree_item_holder child{build_employee_tree(
      source,
...
bool CompanyTreeModel::RemoveEmployeeItem(const QModelIndex& department,
                                          size_t pos) {
  auto* item{get_item(department)};
  if (!item || item->GetType() != ItemType::Department ||
      pos >= item->childCount()) {
...
CompanyTreeModel::Memento CompanyTreeModel::DumpEmployeeItem(
    const QModelIndex& department,
    size_t pos) {
  auto* item{get_item(department)};
  if (!item ||
      item->GetType() !=
          ItemType::Department  //Родительский узел - подразделение
      || pos >= item->childCount()) {
    return Memento();
  }
  Memento::Dump dump{pos, item->childNumber(), std::move((*item)[pos])};
  beginRemoveRows(department, static_cast<int>(pos), static_cast<int>(pos));
...
bool CompanyTreeModel::RestoreEmployeeItem(Memento&& item,
                                           const Employee* new_node) {
  Memento::Dump employee_dump(item.Extract());
  if (new_node) {
    employee_dump.branch->SetWrapperNode(new_node);
  }
  QModelIndex department_q_idx{
      DepartmentIndex(employee_dump.parent_pos.value())};
  auto* department{get_item(department_q_idx)};
  if (!department) {
    return false;
  }
  beginInsertRows(department_q_idx, static_cast<int>(employee_dump.pos),
                  static_cast<int>(employee_dump.pos));
  bool success{department->insertChild(std::move(employee_dump.branch),
                                       employee_dump.pos)};
  endInsertRows();
  return success;
}

CompanyTreeModel::tree_item_holder CompanyTreeModel::build_company_tree(
    const wrapper::Company& company) {
  tree_item_holder root_item{
      CompanyTreeItem::make_instance(ItemType::Company, {})};
  auto range{company.GetDepartments()};  //Диапазон итераторов с сотрудниками
  for (auto it = range.begin(); it != range.end(); ++it) {
    root_item->appendChild(build_department_tree(*it, root_item.get()));
  }
  return root_item;
}

CompanyTreeModel::tree_item_holder CompanyTreeModel::build_department_tree(
    const Department& department,
    CompanyTreeItem* parent) {
  tree_item_holder item{CompanyTreeItem::make_instance(
      ItemType::Department,
      CompanyTreeItem::NodeInfo{QString::fromStdString(department.GetName()),
                                std::addressof(department), std::nullopt},
      parent)};
  auto range{department.GetEmployees()};  //Диапазон итераторов с сотрудниками
  for (auto it = range.begin(); it != range.end(); ++it) {
    item->appendChild(
        build_employee_tree(it->second, department.GetName(), item.get()));
  }
  return item;
}

CompanyTreeModel::tree_item_holder CompanyTreeModel::build_employee_tree(
    const Employee& employee,
    wrapper::string_ref department_name,
    CompanyTreeItem* parent) {
  return CompanyTreeItem::make_instance(
      ItemType::Employee,
      CompanyTreeItem::NodeInfo{FullNameRefToQString(employee.GetFullName()),
                                std::addressof(employee), department_name},
      parent);
}

QString CompanyTreeModel::FullNameRefToQString(
    const wrapper::FullNameRef& name) {
...
CompanyTreeItem* CompanyTreeModel::get_item(const QModelIndex& index) const {
  if (index.isValid()) {
    if (CompanyTreeItem* item =
            static_cast<CompanyTreeItem*>(index.internalPointer());
        item) {
      return item;
    }
  }
