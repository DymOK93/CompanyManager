...
class CompanyTreeModel : public QAbstractItemModel {
 ...
 public:
  ...
  /************************************************************
  Класс Memento хранит ветвь
  и позволяет её извлекать и вставлять без полной пересборки.
  ***********************************************************/
  class Memento {
   public:
    bool IsValid() const noexcept { return m_dump.has_value(); }
   ...
   private:
    ...
    Memento() = default;
    Memento(Dump dump) : m_dump(std::move(dump)) {}
    Dump Extract() {
 ...
 public:
  ...
  std::optional<ItemType> GetItemType(const QModelIndex& index)
      const noexcept;  // nullopt, если индекс не валиден
  std::optional<QString> GetItemName(const QModelIndex& index) const noexcept;
  std::optional<QString> ExchangeItemName(const QModelIndex& index,
                                          QString new_name) noexcept;

  wrapper_node_ptr GetItemNode(const QModelIndex& index);
  std::optional<wrapper::string_ref> GetDepartmentNameRef(
      const QModelIndex&
  ...
  std::optional<size_t> FindChild(
      ...
      const QModelIndex& parent = QModelIndex());  // linear search
  std::optional<size_t> UpperBoundChild(
      const QString& request,
      const QModelIndex& parent =
          QModelIndex());  // binary search for sorted ranges

  std::optional<size_t>
  MoveItem(  //Перемещение потомка parent на новую позицию. Возвращает
             //обновленый индекс позиции с учётом сдвига соседних элементов
  ...
  bool RestoreDepartmentItem(Memento&& item,
                             const Department* new_node = nullptr);

  QModelIndex InsertEmployeeItem(const Employee& source,
  ...
  bool RestoreEmployeeItem(
      Memento&& item,
      const Employee* new_node = nullptr);  //Если узел изменён
 ...
 private:
  ...
  static tree_item_holder build_department_tree(const Department& department,
  ...
  CompanyTreeItem* get_item(const QModelIndex& index)
      const;  //Получение элемента по модельному индексу
 private:
  tree_item_holder m_root_item{
      MakeDummyObjectHolder<CompanyTreeItem>()};  //Корневой элемент
};