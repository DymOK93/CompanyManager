...
namespace task {
enum class ResultType { Success, Fail, EmptyQueue };
}

template <class TargetTy>
class TaskManager {
 public:
  static constexpr size_t DEFAULT_QUEUE_CAPACITY{
      20};  //Стандартная емкость очереди команд
  ...
  using error_log_t =
      std::deque<std::string>;  // deque эффективнее при заранее неизвестном
                                // числе добавленных в конец элементов
 ...
 public:
  TaskManager() = default;
  TaskManager(size_t queue_capacity) : m_queue_capacity(queue_capacity) {}
  ...
  *******************************************************/
  ...
  Result Cancel() {
    ...
    auto [result, success]{//Команду сначала необходимо поместить в очередь
                           //повтора, чтобы избежать рассинхронизации
                           try_to_cancel(get_last_command(m_repeat))};
    if (!success) {
  ...
  Result CancelWithoutQueueing() {  //Отмена без сохранения в очередь repeat
  ...
  Result Repeat() {
    ...
    update_cancel_queue(extract_last_command(m_repeat));
    auto [result, success]{//Команду сначала необходимо поместить в очередь
                           //отмены, чтобы избежать рассинхронизации
                           try_to_execute(get_last_command(m_cancel))};
    if (!success) {
      extract_last_command(m_cancel);
      return make_failed_operation_result();
    }
  ...
  Result RepeatWithoutQueueing() {  //Повтор без сохранения в очередь cancel
                                    //(для нештатных ситуаций)
    if (m_cancel.empty()) {
      return make_empty_queue_operation_result();
    }
  ...
  size_t CanBeCanceled() const noexcept { return m_cancel.size(); }

  size_t CanBeRepeated() const noexcept { return m_repeat.size(); }

  std::optional<command::Type> GetLastProcessedType()
      const noexcept {  //Получение типа последней успешно обработанной команды
  ...
  std::optional<command::Type> GetLastCanceledType()
      const noexcept {  //Получение типа последней отменённой команды
  ...
  std::optional<command::Purpose> GetLastProcessedPurpose() const
      noexcept {  //Получение категории последней успешно обработанной команды
    task_t* command{get_last_command(m_cancel)};
    if (!command) {
      return std::nullopt;
    }
  ...
  std::optional<command::Purpose> GetLastCanceledPurpose()
      const noexcept {  //Получение категории последней отменённой команды
    task_t* command{get_last_command(m_repeat)};
    if (!command) {
      return std::nullopt;
    }
  ...
  void ResetCancelQueue() noexcept { m_cancel.clear(); }

  void ResetRepeatQueue() noexcept { m_repeat.clear(); }
  ...
  const error_log_t& GetErrorLog() const noexcept { return m_error_log; }

  bool Fail() const noexcept { return !m_error_log.empty(); }
 ...
 protected:
  ...
  static Result make_successful_operation_result(std::any&& value) noexcept {
    return {move(value), ResultType::Success};  //Принимаем по r-value ref -
                                                //экономим один вызов move c-tor
  }

 private:
  internal_result_t try_to_process(std::any (task_t::*method)(),
                                   task_t* command) {
  ...
  void log_error_message(std::string msg) { m_error_log.push_back(move(msg)); }
